...
template <class T>
const double PSRoIAlign<T>::samples_[8] = {-0.5, -0.5, -0.5, 0.5, 0.5, -0.5, 0.5, 0.5}; 

#define FLOAT_MAX (128)		// enough

template <class T>
PSRoIAlign<T>::PSRoIAlign(int pnum) : proposal_num(pnum) {
	...
	rlt_sampled_.resize(proposal_num * roi_pool_h_ * roi_pool_w_ * class_num_ * bisample_num_);
}

template <class T>
void PSRoIAlign<T>::genInputFmap() {
		...
		for (int ph = 0; ph < roi_pool_h_; ++ph) {
			for (int pw = 0; pw < roi_pool_w_; ++pw) {
				...
				for(int c = 0; c < class_num_; c++) {
					int pool_index =  rlt_addr_base + c;
...
template <class T>
void PSRoIAlign<T>::pool() {
	int h_base = roi_pool_w_ * class_num_;
	int p_base = roi_pool_h_ * h_base;
	for(int p = 0; p < proposal_num; p++) {
		for(int h = 0; h < roi_pool_h_; h++) {
			for(int w = 0; w < roi_pool_w_; w++) {
				for(int c = 0; c < class_num_; c++) {
					int pool_index = p * p_base + h * h_base + w * class_num_ + c;
					if(!pool_type_.compare("max")) {
						rlt[pool_index] = -FLOAT_MAX;
						for(int r = 0; r < 4; r++) {
...
template <class T>
void PSRoIAlign<T>::printInputFmap(int channel) {
...
template <class T>
void PSRoIAlign<T>::printProposal(int proposal_id) {
...
template <class T>
void PSRoIAlign<T>::printRoI(int proposal_id) {
	if(proposal_id < 0 || proposal_id >= proposal_num) {
		std::cout<<"invalid proposal_id\n";
		abort();
	}
...
template <class T>
void PSRoIAlign<T>::printOutputFmap(int proposal_id, int channel) {
	if(proposal_id < 0 || proposal_id >= proposal_num) {
		std::cout<<"invalid proposal_id\n";
		abort();
	}
...
template class PSRoIAlign<double>;
template class PSRoIAlign<int>;
